...
typedef struct camera_format
{
    ...
    unsigned char pixelformat;  //像素格式
}cam_fmt;
...
/*初始化摄像头*/
int v4l2_dev_init(const char *dev)
{
    struct v4l2_capability cap; //描述设备信息结构体
    /*打开摄像头*/
    if(0 > (v4l2_fd = open(dev, O_RDONLY)))
    {
        perror("v4l2 dev open fail");
        return -1;
    }
        ...
        while (0 == ioctl(v4l2_fd, VIDIOC_ENUM_FRAMESIZES, &frmsize))
         {
            ...
            {
                printf("size<%d*%d> ",frmival.discrete.denominator,frmival.discrete.numerator);
                frmival.index++;
            }
            printf("\n");
         }
...
int v4l2_set_format(void)
{
    ...
    /*** 判断是否已经设置为我们要求的 RGB565 像素格式,如果没有设置成功表示该设备不支持 RGB565 像素格式 */
...
int v4l2_init_buffer(void)
{
    ...
    {
        ioctl(v4l2_fd, VIDIOC_QUERYBUF, &buf); //获取帧缓冲信息
